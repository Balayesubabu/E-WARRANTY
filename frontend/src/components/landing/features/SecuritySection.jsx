          ...
          <div className="grid grid-cols-2 gap-4 max-w-md mx-auto lg:mx-0 items-stretch">
            {securityCards.map((card) => {
              ...
              return (
                <div
                  key={card.title}
                  className="flex flex-col bg-white rounded-2xl p-6 shadow-sm border border-sky-100 hover:shadow-md transition-shadow"
                >
            ...
            <ul className="space-y-4">
              {benefits.map((item) => (
                <li key={item} className="flex items-start gap-3">
                  <span className="w-6 h-6 rounded-full bg-sky-100 flex items-center justify-center shrink-0 mt-0.5">
