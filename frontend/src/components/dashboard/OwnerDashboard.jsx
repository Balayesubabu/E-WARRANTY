...
const statusClass = (status) => {
  ...
  if (key === 'resolved') return 'bg-[#1A7FC1]/15 text-[#1A7FC1]';
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      if (results[0].status === 'fulfilled') {
        const dealersData = results[0].value?.data || [];
      ...
      if (results[1].status === 'fulfilled') {
        const warrantyCodes = results[1].value?.data?.warranty_codes || [];
      ...
      if (results[2].status === 'fulfilled') {
        const resData = results[2].value?.data?.data;
      ...
      if (results[3].status === 'fulfilled') {
        const staffList = results[3].value?.data?.data || [];
      ...
      if (results[4].status === 'fulfilled') {
        const ticketsRaw = results[4].value?.data || [];
      ...
      const pendingClaims = customerList.filter((c) => {
        const status = String(c?.provider_warranty_code?.warranty_code_status || c?.status || '').toLowerCase();
        return status === 'pending' || (!status && c?.is_active !== true);
      }).length;

      const activeCustomers = customerList.filter((c) => {
        const status = String(c?.provider_warranty_code?.warranty_code_status || c?.status || '').toLowerCase();
        return status === 'active' || c?.is_active === true;
      }).length;

      setStats({ totalWarranties, activeDealers: activeDealerCount, totalCustomers: uniqueCustomerCount, totalStaff, pendingClaims, activeCustomers, pendingTickets });
      setDashboardMeta({ totalDealers: dealerList.length, inactiveDealers: inactiveDealerCount, failedSources: results.filter((r) => r.status === 'rejected').length, lastUpdated: new Date().toISOString() });
    } catch (error) {
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
        <div className="xl:col-span-2 space-y-5">
          ...
          <div className="rounded-xl bg-white border border-slate-200 p-4 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 mt-3">
              <button onClick={() => navigate('/owner/register-customer')} className="h-10 rounded-lg bg-[#1A7FC1] text-white text-sm hover:bg-[#166EA8]">Register Customer</button>
              <button onClick={() => navigate('/verify')} className="h-10 rounded-lg border border-slate-300 bg-white text-[#1A7FC1] text-sm hover:bg-slate-50">Verify a Product</button>
              <button onClick={() => navigate('/owner/dealer-management')} className="h-10 rounded-lg border border-slate-300 bg-white text-[#1A7FC1] text-sm hover:bg-slate-50">Manage Dealers</button>
              <button onClick={() => navigate('/owner/staff')} className="h-10 rounded-lg border border-slate-300 bg-white text-[#1A7FC1] text-sm hover:bg-slate-50">Manage Staff</button>
            </div>
          </div>
          ...
          <div className="rounded-xl bg-white border border-slate-200 overflow-hidden shadow-sm">
            <div className="px-4 py-3 border-b border-slate-200 flex items-center justify-between">
              ...
              <button onClick={() => navigate('/owner/warranty-claims')} className="text-sm text-[#1A7FC1] hover:text-[#166EA8]">View All</button>
            </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {filteredClaims.length === 0 ? (
                    <tr><td colSpan={5} className="px-4 py-8 text-center text-slate-400">No claim records available</td></tr>
                  ) : (
                    filteredClaims.slice(0, 8).map((customer, idx) => {
                      ...
                      return (
                        <tr key={customer.id || idx} className="border-t border-slate-100">
                          ...
                          <td className="px-4 py-3 text-[#0F4E78]">{`${customer.first_name || ''} ${customer.last_name || ''}`.trim() || 'Customer'}</td>
                          <td className="px-4 py-3 text-[#1A7FC1]">{customer.product_name || '-'}</td>
                          <td className="px-4 py-3"><span className={`px-2 py-0.5 rounded-full text-xs ${statusClass(claimStatus)}`}>{claimStatus}</span></td>
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2">
                              <button onClick={() => navigate('/owner/warranty-claims')} className="px-3 h-8 rounded-md bg-[#1A7FC1] hover:bg-[#166EA8] text-white text-xs">Approve</button>
                              <button onClick={() => navigate('/owner/warranty-claims')} className="px-3 h-8 rounded-md border border-slate-300 bg-white hover:bg-slate-50 text-[#1A7FC1] text-xs">Review</button>
                            </div>
                          </td>
                        </tr>
                      );
        ...
        <div className="space-y-5">
          ...
          <div className="rounded-xl bg-white border border-slate-200 p-4 shadow-sm">
                  ...
                  return (
                    <div key={item.month} className="flex flex-col items-center justify-end gap-1 h-full">
                      <div className="w-full flex items-end justify-center gap-1 h-full">
                        <div className="w-1.5 rounded bg-[#1A7FC1]/80" style={{ height: `${Math.max(10, Math.round((item.claims / maxVal) * 100))}%` }} />
                        <div className="w-1.5 rounded bg-[#1A7FC1]/50" style={{ height: `${Math.max(10, Math.round((item.registrations / maxVal) * 100))}%` }} />
                        <div className="w-1.5 rounded bg-[#1A7FC1]/25" style={{ height: `${Math.max(10, Math.round((item.dealers / maxVal) * 100))}%` }} />
                      </div>
            ...
            <button onClick={() => navigate('/owner/analytics')} className="mt-3 w-full h-10 rounded-lg bg-[#1A7FC1] hover:bg-[#166EA8] text-white">View Report</button>
          </div>
          ...
          <div className="rounded-xl bg-white border border-slate-200 p-4 shadow-sm">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => navigate('/owner/dealer-management')} className="text-xs text-[#1A7FC1] hover:text-[#166EA8]">View All</button>
            </div>
          ...
          <div className="rounded-xl bg-white border border-slate-200 p-4 shadow-sm">
            ...
            <div className="space-y-2">
              <button onClick={() => navigate('/owner/register-customer')} className="w-full h-9 rounded-lg border border-slate-300 bg-white hover:bg-slate-50 text-[#1A7FC1] text-sm flex items-center justify-between px-3">
                Register Customer <ChevronRight className="w-4 h-4" />
              </button>
              <button onClick={() => navigate('/owner/warranty-codes')} className="w-full h-9 rounded-lg border border-slate-300 bg-white hover:bg-slate-50 text-[#1A7FC1] text-sm flex items-center justify-between px-3">
                Manage Warranty Codes <ChevronRight className="w-4 h-4" />
              </button>
              <button onClick={() => navigate('/owner/staff')} className="w-full h-9 rounded-lg border border-slate-300 bg-white hover:bg-slate-50 text-[#1A7FC1] text-sm flex items-center justify-between px-3">
                Manage Staff <ChevronRight className="w-4 h-4" />
              </button>
              <button onClick={() => navigate('/owner/support-tickets')} className="w-full h-9 rounded-lg border border-slate-300 bg-white hover:bg-slate-50 text-[#1A7FC1] text-sm flex items-center justify-between px-3">
