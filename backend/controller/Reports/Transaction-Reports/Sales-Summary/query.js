...
const getSalesInvoices = async (provider_id, franchise_id, provider_customer_id, invoice_payment_status,start_date, end_date) => {
    let startDateFilter = new Date(start_date);
    let endDateFilter = new Date(end_date);

    const getSalesInvoices = await SalesInvoice.findMany({
        where: {
    ...
    const formattedInvoices = getSalesInvoices.map((inv, index) => ({
    Sr_No: index + 1,
    ...inv,
...
export { getProviderByUserId, getSalesInvoices };