...
const updateProviderCustomer = async (data, customer_id, provider_id) => {
  try {
    const providerCustomer = await ProviderCustomers.update({
      where: {
        id: customer_id,
        provider_id: provider_id, // ✅ Ensures customer belongs to provider
      },
      data: {
    ...
    console.error("❌ Prisma Error in updateProviderCustomer:", err);
    throw err;
  }
};

export { getProviderByUserId, updateProviderCustomer };
