...
const generateApiKeyEndpoint = async (req, res) => {
    try {
        ...
        const updatedUser = await updateUser(user_id, { api_key: apiKey, is_api_key_generated: true, is_api_key_active: true });
        if (!updatedUser) {
        ...
        return returnError(res, StatusCodes.INTERNAL_SERVER_ERROR, `Error in generateApiKeyEndpoint: ${error}`);
    }
}
