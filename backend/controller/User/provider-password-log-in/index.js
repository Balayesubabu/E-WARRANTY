...
import { getUserByEmailOrPhoneNumber, checkUserSubscription, getFranchisesByProviderId,getProviderByUserId} from "./query.js";   

const providerLogIn = async (req, res) => {
    try {
        logger.info("providerLogIn")
        ...
        if (!isPasswordCorrect) {
            logger.info(`--- Password is incorrect by user with email ${email} or phone number ${phone_number} and user_id ${user.id} ---`);
            return returnError(res, StatusCodes.UNAUTHORIZED, "Invalid password");
        }
        logger.info(`--- Password is correct for user with email ${email} or phone number ${phone_number} and user_id ${user.id} ---`);
        ...
        return returnResponse(res, StatusCodes.OK, "User found and password is correct", { token, isSubscriptionActive, franchiseOne });
    } catch (error) {
        console.log(error);
        logger.error(`--- Error in providerLogIn ${error} ---`);
