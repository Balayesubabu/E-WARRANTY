...
const getAllProviderFranchise = async (req, res) => {
    try {
        ...
        logger.info(`--- Fetching all franchises for provider ${provider.id} ---`);
        const franchises = await getFranchisesByProviderId(provider.id);
        if (!franchises) {
        ...
        return returnResponse(res, StatusCodes.OK, `All franchises fetched successfully`, franchises);
    } catch (error) {
        return returnError(res, StatusCodes.INTERNAL_SERVER_ERROR, error);
    }
}

export { getAllProviderFranchise };