...
const deliveryToSalesInvoiceEndpoint = async (req, res) => {
    try {
        logger.info(`deliveryToSalesInvoiceEndpoint`);
        ...
        const deliveryChallan = await getSalesInvoiceById(provider.id, franchise_id,sales_invoice_id);
        if (!deliveryChallan) {
        ...
        logger.info(`--- delivery challan found for sales_invoice_id: ${sales_invoice_id} ---`);

        logger.info(`--- Checking if sales invoice is already converted to sales invoice ---`);
        if (deliveryChallan.invoice_status === "Delivery_Challan_Converted") {
            logger.error(`--- Sales invoice is already converted to sales invoice ---`);
            return returnError(res, StatusCodes.BAD_REQUEST, "Delivery challan is already converted to sales invoice");
        }


           const bill_to = deliveryChallan.SalesInvoiceParty.find(party => party.type === "Bill_To");
    const ship_to = deliveryChallan.SalesInvoiceParty.find(party => party.type === "Ship_To");
    ...
    console.log("ship_to", ship_to);
        logger.info(`--- Starting transaction for delivery challan conversion ---`);
        const result = await prisma.$transaction(async (tx) => {

            logger.info(`--- Updating sales invoice status to Delivery_Challan_Converted ---`);
            const updatedDeliveryChallan = await updateSalesInvoiceStatus(sales_invoice_id, tx);
      ...
      const sales_invoice = await createSalesInvoice(provider.id, staff_id,deliveryChallan,invoice_number,tx);
     ...
     logger.info(`--- Updating franchise inventory ---`);
        ...
        await createSalesServices(
          ...
          tx
            ...
            await updateCustomerBalance(
                deliveryChallan.provider_customer_id,
                deliveryChallan.invoice_total_amount,
                tx
            );
      ...
      const updated_invoice = await updateUrl(salesInvoiceId, s3Url);
        ...
        logger.info(`--- Delivery challan converted to sales invoice successfully ---`);
        return returnResponse(res, StatusCodes.OK, "Delivery challan converted to sales invoice successfully", result);

    } catch (error) {
        logger.error(`Error in deliveryToSalesInvoiceEndpoint: ${error.message}`);
        return returnError(res, StatusCodes.INTERNAL_SERVER_ERROR, "Internal server error", error.message);
    }
};

export { deliveryToSalesInvoiceEndpoint };