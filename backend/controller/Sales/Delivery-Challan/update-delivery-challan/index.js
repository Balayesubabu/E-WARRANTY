...
import { getProviderByUserId, getFranchiseInventoryById, getSalesInvoiceById, updateSalesInvoice, updateSalesInvoiceParty, deleteSalesParts, deleteSalesServices, deleteSalesAdditionalCharges, createSalesParts, createSalesServices, createSalesAdditionalCharges,
...
const updateDeliveryChallanEndpoint = async (req, res) => {
    try {
        logger.info(`updateDeliveryChallanEndpoint`);
        ...
        logger.info(`--- Fetching existing delivery challan ---`);
        const existingDeliveryChallan = await getSalesInvoiceById(sales_invoice_id);
        if (!existingDeliveryChallan) {
            logger.error(`--- Delivery challan not found ---`);
            return returnError(res, StatusCodes.NOT_FOUND, `Delivery challan not found`);
        }
        logger.info(`--- Delivery challan found ---`);

        // Validate that it's actually a delivery challan
        if (existingDeliveryChallan.invoice_type !== "Delivery_Challan") {
            logger.error(`--- Invalid invoice type: ${existingDeliveryChallan.invoice_type} ---`);
            return returnError(res, StatusCodes.BAD_REQUEST, `Invalid invoice type. Expected Delivery_Challan but found ${existingDeliveryChallan.invoice_type}`);
        }

         const old_parts = existingDeliveryChallan.SalesPart || [];
    logger.info(`--- Old parts: ${JSON.stringify(old_parts)} ---`);

    const transaction_id = existingDeliveryChallan?.Transaction?.[0]?.id || null;
    ...
    if (is_auto_round_off) {
      ...
      invoice_total_amount += auto_round_off_amount;
        ...
        const result = await prisma.$transaction(async (tx) => {
            logger.info(`--- Starting transaction for delivery challan update ---`);

            logger.info(`--- Updating delivery challan ---`);
            const updated_delivery_challan = await updateSalesInvoice(sales_invoice_id,provider.id,staff_id, {
                provider_customer_id,
                franchise_id,
                invoice_number : existingDeliveryChallan.invoice_number,
                prefix,
                sequence_number,
                invoice_type,
                invoice_status,
      ...
      bank_id: data.bank_id ? data.bank_id : existingDeliveryChallan.bank_id
            }, tx);

            if (!updated_delivery_challan) {
                throw new Error('Failed to update delivery challan');
            }
            logger.info(`--- Delivery challan updated ---`);

            logger.info(`--- Updating provider party ---`);
            const invoice_provider_data = await updateSalesInvoiceParty(sales_invoice_id, "Provider", {
            ...
            if (!invoice_provider_data) {
                throw new Error('Failed to update provider party');
            }
            logger.info(`--- Provider party updated ---`);
          ...
          tx
            ...
            logger.info(`--- Creating new additional charges ---`);
        ...
        return returnResponse(res, StatusCodes.OK, "Delivery challan updated successfully", result);

    } catch (error) {
        logger.error(`Error in updateDeliveryChallanEndpoint: ${error}`);
        return returnError(res, StatusCodes.INTERNAL_SERVER_ERROR, `Error in updateDeliveryChallanEndpoint: ${error.message}`);
    } finally {
        await prisma.$disconnect();
    }
}

export { updateDeliveryChallanEndpoint };   