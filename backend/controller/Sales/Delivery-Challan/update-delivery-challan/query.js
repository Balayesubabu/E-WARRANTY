...
const updateSalesInvoice = async (sales_invoice_id,provider_id,staff_id, data, tx) => {
...
const updateSalesInvoiceParty = async (sales_invoice_id, type, data, tx) => {
    // First find the party record
    const existingParty = await tx.salesInvoiceParty.findFirst({
        where: {
            AND: [
    ...
    if (!existingParty) {
        throw new Error(`Party of type ${type} not found for sales invoice ${sales_invoice_id}`);
    }

    // Then update using the found id
    const salesInvoiceParty = await tx.salesInvoiceParty.update({
        where: {
            id: existingParty.id
        },
        data: {
...
const createTransactions = async (created_delivery_challan_id,transaction_id, data, tx) => {
