...
const updateSalesReturnEndpoint = async (req, res) => {
    try {
        logger.info(`updateSalesReturnEndpoint`);
        ...
        logger.info(`--- Fetching sales return details for sales_invoice_id: ${sales_invoice_id} ---`);
        const existing_sales_invoice = await getSalesInvoiceById(sales_invoice_id);
        if (!existing_sales_invoice) {
            logger.error(`--- Sales return not found for sales_invoice_id: ${sales_invoice_id} ---`);
            return returnError(res, StatusCodes.NOT_FOUND, `Sales return not found`);
        }
        logger.info(`--- Sales return found for sales_invoice_id: ${sales_invoice_id} ---`);

        logger.info(`--- Fetching sales return transactions for sales_invoice_id: ${sales_invoice_id} ---`);
        const existing_transactions = await getSalesInvoiceTransactions(sales_invoice_id);
        if (!existing_transactions) {
            logger.error(`--- Sales return transactions not found for sales_invoice_id: ${sales_invoice_id} ---`);
            return returnError(res, StatusCodes.NOT_FOUND, `Sales return transactions not found`);
        }
        logger.info(`--- Sales return transactions found for sales_invoice_id: ${sales_invoice_id} ---`);
        ...
        logger.info(`--- Clearing sales return for sales_invoice_id: ${sales_invoice_id} ---`);
        await clearSalesInvoice(sales_invoice_id);
        logger.info(`--- Sales return cleared for sales_invoice_id: ${sales_invoice_id} ---`);
...
for (const charge of SalesAdditionalCharges) {
  ...
  logger.info(`invoice discount amount: ${invoice_discount_amount}`);
    ...
    if(is_taxable_amount){
      ...
      // invoice_total_tax_amount += invoice_tcs_amount;
      invoice_tcs_amount = (invoice_total_parts_services_amount - invoice_total_parts_services_tax_amount + invoice_discount_amount - invoice_additional_discount_amount) * (invoice_tcs_percentage / 100);
...
if (apply_tds === true) {
    ...
    total_amount_payable = invoice_pending_amount
        ...
        const result = await prisma.$transaction(async (tx) => {
            logger.info(`--- Starting transaction for sales return update ---`);

            logger.info(`--- Updating sales return ---`);
            const updated_sales_invoice = await updateSalesInvoice(provider.id,staff_id,sales_invoice_id, {
                provider_customer_id,
                franchise_id,
                bill_to,
                ship_to,
                // original_invoice_number,
                invoice_number : existing_sales_invoice.invoice_number,
                prefix,
                sequence_number,
                invoice_status : existing_sales_invoice.invoice_status,
                invoice_date,
                is_invoice_fully_paid,
                invoice_additional_discount_percentage,
                invoice_additional_discount_amount,
            ...
            if (!updated_sales_invoice) {
                throw new Error('Failed to update sales return');
            }
            logger.info(`--- Sales return updated ---`);
    ...
    if(salesInvoice.linked_from_pending_amount === 0){
      ...
      const updated_invoice1 = await updateUrl(salesInvoiceId1, s3Url1, updated_payment_status);
        ...
        return returnResponse(res, StatusCodes.OK, "Sales return updated successfully", result);

    } catch (error) {
        logger.error(`Error in updateSalesReturnEndpoint: ${error}`);
        return returnError(res, StatusCodes.INTERNAL_SERVER_ERROR, `Error in updateSalesReturnEndpoint: ${error.message}`);
    } finally {
        await prisma.$disconnect();
    }
}

export { updateSalesReturnEndpoint };
