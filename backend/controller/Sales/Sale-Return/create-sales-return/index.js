...
const createSalesReturnEndpoint = async (req, res) => {
  try {
    logger.info(`createSalesReturnEndpoint`);
...
for (const charge of SalesAdditionalCharges) {
  ...
  if (apply_tds === true) {
    ...
    // const invoice_number = await generateInvoiceNo(provider.id, "Sales_Return");
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      // if (
      //   is_invoice_fully_paid ||
      //   (invoice_advance_amount && !is_invoice_fully_paid)
      // ) {
      //   const amount_to_record = is_invoice_fully_paid
      //     ? invoice_total_amount
      //     : invoice_advance_amount;
    ...
    logger.error(`Error in createSalesReturnEndpoint: ${error}`);
    return returnError(
      res,
      StatusCodes.INTERNAL_SERVER_ERROR,
      `Error in createSalesReturnEndpoint: ${error.message}`
    );
  } finally {
...
export { createSalesReturnEndpoint };
