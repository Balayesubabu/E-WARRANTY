...
const createProviderBankDetails = async (sales_invoice_id, data, tx) => {
  const bankDetails = await tx.providerBankDetails.create({
    data: {
      provider_id: data.provider_id, // must come from provider
...
const updateProviderCustomerFinalBalance = async (
  ...
  const providerCustomer = await tx.providerCustomers.update({
    ...
    data: {
      customer_final_balance: {
        increment: customer_final_balance,
      },
    },
  });
...
const createFranchiseOpenInventoryTransaction = async ( provider_id,data, tx) => {
  const inventoryTransaction = await tx.franchiseOpenInventoryTransaction.create({
    data: {
      provider_id : provider_id,
      franchise_id :data.franchise_id,
...
const createTransactions = async (sales_invoice_id, data, tx) => {
