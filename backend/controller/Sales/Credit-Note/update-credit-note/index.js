...
const updateCreditNoteEndpoint = async (req, res) => {
    try {
        logger.info(`updateCreditNoteEndpoint`);
       ...
       let user_id;
        ...
        const data = req.body;
        const sales_invoice_id = req.params.credit_note_id;

        logger.info(`--- Fetching credit note details for sales_invoice_id: ${sales_invoice_id} ---`);
        const existing_credit_note = await getSalesInvoiceById(sales_invoice_id);
        if (!existing_credit_note) {
            logger.error(`--- Credit note not found for sales_invoice_id: ${sales_invoice_id} ---`);
            return returnError(res, StatusCodes.NOT_FOUND, `Credit note not found`);
        }
        logger.info(`--- Credit note found for sales_invoice_id: ${sales_invoice_id} ---`);

        logger.info(`--- Fetching credit note transactions for sales_invoice_id: ${sales_invoice_id} ---`);
        const existing_transactions = await getSalesInvoiceTransactions(sales_invoice_id);
        if (!existing_transactions) {
            logger.error(`--- Credit note transactions not found for sales_invoice_id: ${sales_invoice_id} ---`);
            return returnError(res, StatusCodes.NOT_FOUND, `Credit note transactions not found`);
        }
        logger.info(`--- Credit note transactions found for sales_invoice_id: ${sales_invoice_id} ---`);
        ...
        const old_parts = existing_credit_note.SalesPart || [];
        ...
        logger.info(`--- Clearing credit note for sales_invoice_id: ${sales_invoice_id} ---`);
        await clearSalesInvoice(sales_invoice_id);
        logger.info(`--- Credit note cleared for sales_invoice_id: ${sales_invoice_id} ---`);


        const transaction_id = existing_credit_note?.Transaction?.[0]?.id || null;
...
for (const charge of SalesAdditionalCharges) {
  ...
  logger.info(`invoice discount amount: ${invoice_discount_amount}`);
    ...
    total_amount_payable = invoice_pending_amount
        ...
        const invoice_number= existing_credit_note.invoice_number
        logger.info(`--- Wrapping all database operations in a transaction ---`);
        const result = await prisma.$transaction(async (tx) => {
            logger.info(`--- Starting transaction for credit note update ---`);

            logger.info(`--- Updating credit note ---`);
            const updated_credit_note = await updateSalesInvoice(provider.id, staff_id,sales_invoice_id, {
                provider_customer_id,
                sequence_number,
                prefix,
                franchise_id,
                ...
                advance_payment_type,
                advance_amount_online_transaction_id,
                advance_payment_date: advance_payment_date ? new Date(advance_payment_date) : null,
                invoice_payment_status,
                terms_and_conditions,
                additional_notes,
                due_date_terms,
                ...
                apply_tcs,
                apply_tds,
                bank_id : bank_id ? bank_id : existing_credit_note.bank_id,
        is_total_amount,
        is_taxable_amount,
        link_to
            }, tx);

            if (!updated_credit_note) {
                throw new Error('Failed to update credit note');
            }
            logger.info(`--- Credit note updated ---`);

            logger.info(`--- Updating customer final balance ---`);
            const balance_difference = customer_final_balance - existing_credit_note.invoice_pending_amount;
            ...
            if (is_invoice_fully_paid === true) {
                await createSalesInvoiceTransactions(sales_invoice_id, {
                    invoice_type: "Credit_Note",
                    ...
                    transaction_id: advance_amount_online_transaction_id ? advance_amount_online_transaction_id : ""
                }, tx);
            ...
            console.log("creating transaction with id", transaction_id)
            await createTransactions(
          existing_credit_note.id,transaction_id,
          {
    ...
    if(salesInvoice.linked_from_pending_amount === 0){
      ...
      const updated_invoice1 = await updateUrl(salesInvoiceId1, s3Url1, updated_payment_status);
        ...
        return returnResponse(res, StatusCodes.OK, "Credit note updated successfully", result);

    } catch (error) {
        logger.error(`Error in updateCreditNoteEndpoint: ${error}`);
        return returnError(res, StatusCodes.INTERNAL_SERVER_ERROR, `Error in updateCreditNoteEndpoint: ${error.message}`);
    } finally {
        await prisma.$disconnect();
    }
}

export { updateCreditNoteEndpoint };
