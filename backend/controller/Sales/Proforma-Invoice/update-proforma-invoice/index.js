...
import { getProviderByUserId, getFranchiseInventoryById, getSalesInvoiceById, updateSalesInvoice, updateSalesInvoiceParty, deleteSalesParts, deleteSalesServices, deleteSalesAdditionalCharges, createSalesParts, createSalesServices, createSalesAdditionalCharges, getAllDataSalesInvoiceById, updateUrl } from "./query.js";
...
const updateProformaInvoiceEndpoint = async (req, res) => {
    try {
        logger.info(`updateProformaInvoiceEndpoint`);
        ...
        logger.info(`--- Fetching existing proforma invoice ---`);
        const existingProformaInvoice = await getSalesInvoiceById(sales_invoice_id);
        if (!existingProformaInvoice) {
            logger.error(`--- Proforma invoice not found ---`);
            return returnError(res, StatusCodes.NOT_FOUND, `Proforma invoice not found`);
        }
        logger.info(`--- Proforma invoice found ---`);

        // Validate that it's actually a proforma invoice
        if (existingProformaInvoice.invoice_type !== "Proforma_Invoice") {
            logger.error(`--- Invalid invoice type: ${existingProformaInvoice.invoice_type} ---`);
            return returnError(res, StatusCodes.BAD_REQUEST, `Invalid invoice type. Expected Proforma_Invoice but found ${existingProformaInvoice.invoice_type}`);
        }
    ...
    if (is_auto_round_off) {
      ...
      invoice_total_amount += auto_round_off_amount;
        ...
        const result = await prisma.$transaction(async (tx) => {
            logger.info(`--- Starting transaction for proforma invoice update ---`);

            logger.info(`--- Updating proforma invoice ---`);
            const updated_proforma_invoice = await updateSalesInvoice(provider.id, staff_id,sales_invoice_id , {
                provider_customer_id,
                franchise_id,
                invoice_number : existingProformaInvoice.invoice_number,
                prefix,
                sequence_number,
                invoice_type,
                invoice_status,
      ...
      invoice_discount_amount,
        bank_id : bank_id  ? bank_id : existingProformaInvoice.bank_id
            }, tx);

            if (!updated_proforma_invoice) {
                throw new Error('Failed to update proforma invoice');
            }
            logger.info(`--- Proforma invoice updated ---`);
            ...
            return updated_proforma_invoice;
        });
        ...
        return returnResponse(res, StatusCodes.OK, "Proforma invoice updated successfully", result);

    } catch (error) {
        logger.error(`Error in updateProformaInvoiceEndpoint: ${error}`);
        return returnError(res, StatusCodes.INTERNAL_SERVER_ERROR, `Error in updateProformaInvoiceEndpoint: ${error.message}`);
    } finally {
        await prisma.$disconnect();
    }
}

export { updateProformaInvoiceEndpoint };   