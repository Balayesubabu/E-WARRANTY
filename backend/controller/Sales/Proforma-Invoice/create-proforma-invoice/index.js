...
const createProformaInvoiceEndpoint = async (req, res) => {
  try {
    logger.info(`createProformaInvoiceEndpoint`);
...
for (const charge of SalesAdditionalCharges) {
  ...
  console.log(`invoice_total_tax_amount after additional charges: ${invoice_total_tax_amount}`);
    ...
    const result = await prisma.$transaction(async (tx) => {
      logger.info(`--- Starting transaction for proforma invoice creation ---`);

      logger.info(`--- Creating proforma invoice ---`);
      const created_proforma_invoice = await createSalesInvoice(
      ...
      if (!created_proforma_invoice) {
        throw new Error("Failed to create proforma invoice");
      }
      ...
      for (const part of processed_parts) {
        ...
        await createSalesParts(
          ...
          {
            ...part,
            sales_invoice_id: created_proforma_invoice.id,
          },
          tx
        );
      }
      ...
      return created_proforma_invoice;
    });
    ...
    logger.error(`Error in createProformaInvoiceEndpoint: ${error}`);
    return returnError(
      res,
      StatusCodes.INTERNAL_SERVER_ERROR,
      `Error in createProformaInvoiceEndpoint: ${error.message}`
    );
  } finally {
...
export { createProformaInvoiceEndpoint };
