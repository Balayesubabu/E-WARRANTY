...
const getPurchaseInvoiceByCustomerForPaymentOutEndPoint = async (req, res) => {
    try {
            let user_id;
    let staff_id;
    if (req.type === "staff") {
    ...
    const franchise_id = req.franchise_id;
        ...
        const purchaseinvoice = await getPurchaseInvoiceByCustomer(provider.id, provider_customer_id, franchise_id);
        if(!purchaseinvoice ) {
    ...
    for (let i = 0; i < purchaseinvoice.length; i++) {
      ...
      if(!result){
        ...
        purchaseinvoice[i].linked_invoice_total_amount = 0;
        purchaseinvoice[i].updated_pending_amount = (purchaseinvoice[i].invoice_pending_amount).toFixed(2);
      }
        else{   
        purchaseinvoice[i].is_linked = true;
        purchaseinvoice[i].linked_invoice_id = result.id;
        purchaseinvoice[i].linked_invoice = result;
        purchaseinvoice[i].linked_invoice_total_amount = result.invoice_total_amount;
        purchaseinvoice[i].updated_pending_amount = (purchaseinvoice[i].invoice_pending_amount - result.invoice_total_amount).toFixed(2);
      }
    }
...
export { getPurchaseInvoiceByCustomerForPaymentOutEndPoint };