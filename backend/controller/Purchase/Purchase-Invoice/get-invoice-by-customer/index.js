...
import { getProviderByUserId, getPurchaseInvoiceByCustomerId } from "./query.js";
...
const getPurchaseInvoiceByCustomerIdEndpoint = async (req, res) => {
  try {
    ...
    const purchaseInvoice = await getPurchaseInvoiceByCustomerId(customer_id, franchise_id, staff_id, provider.id);
    if (!purchaseInvoice) {
      logger.error(
        `--- Purchase invoice not found for customerId: ---`
      );
      return returnError(
        res,
