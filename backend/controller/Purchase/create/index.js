...
const createPurchaseEndpoint = async (req, res) => {
  try {
    let user_id;
    let staff_id;
    if (req.type === "staff") {
    ...
    for (const part of PurchaseParts) {
      ...
      console.log("invoice_total_parts_services_discount_amount", invoice_total_parts_services_discount_amount);
    ...
    for (const charge of PurchaseAdditionalCharges) {
      ...
      invoice_total_amount += Number(total_amount.toFixed(2));
   ...
   if (apply_tcs === true) {
    ...
    } else if (is_invoice_fully_paid === false && invoice_advance_amount > 0 ) {
...
console.log("TX KEYS:", Object.keys(tx));
      ...
      const created_purchase = await createPurchaseInvoice(
        provider.id,
        franchise_id,
        staff_id,
        {
      ...
      const invoice_provider_data = await createPurchaseInvoiceParty(
        created_purchase.id,
        {
          purchase_invoice_id: created_purchase.id,
          type: "Provider",
      ...
      for (const part of processed_parts) {
        ...
        await createFranchiseInventoryTransaction(
          provider.id,created_purchase.id,{
      ...
      console.log("processed_additional_charges", processed_additional_charges);
...
if(invoice_type === "Debit_Note" || invoice_type === "Purchase_Return"){
    ...
    logger.error(`Error in createPurchaseInvoiceEndpoint: ${error}`);
    return returnError(
      res,
