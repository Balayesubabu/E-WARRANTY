...
const getPurchaseInvoiceByIdEndpoint = async (req, res) => {
  try {
    ...
    const {invoice_type , id}= req.params;

 
    const purchaseInvoice = await getPurchaseInvoiceById(id, provider.id, franchise_id, invoice_type);
    if (!purchaseInvoice) {
...
const downloadPurchaseInvoiceByIdEndpoint = async (req, res) => {
  try {
   ...
   const purchaseInvoice = await getPurchaseInvoiceById(id, provider.id, franchise_id, staff_id, invoice_type);
    if (!purchaseInvoice) {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="${invoice_type}_${id}.pdf"`
    );
...
export { getPurchaseInvoiceByIdEndpoint, downloadPurchaseInvoiceByIdEndpoint };
