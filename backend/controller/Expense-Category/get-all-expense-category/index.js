import  {getProviderByUserId ,  getAllExpenseCategory} from './query.js'
import {logger, returnError, returnResponse} from '../../../services/logger.js'
import {StatusCodes} from 'http-status-codes'
import {ExpenseCategory} from '../../../prisma/db-models.js'

const getAllExpenseCategoryEndpoint = async (req , res) => {
    try {
        ...
        const expense_category = await getAllExpenseCategory(provider.id, franchise_id);
        if (!expense_category) {
        ...
        return returnResponse(res, StatusCodes.OK, `expense_category fetched successfully`, expense_category);
    } catch (error) {
        return returnError(res, StatusCodes.INTERNAL_SERVER_ERROR, error.message);
    }
}

export {getAllExpenseCategoryEndpoint}