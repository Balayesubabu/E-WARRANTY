import  {getProviderByUserId ,  getExpenseCategoryById ,  deleteExpenseCategoryById} from './query.js'      
import {logger, returnError, returnResponse} from '../../../services/logger.js'
import {StatusCodes} from 'http-status-codes'
import {ExpenseCategory} from '../../../prisma/db-models.js'

const deleteExpenseCategoryByIdEndpoint = async (req , res) => {
    try {
        ...
        const franchise_id = req.franchise_id;
         ...
         const expense_category = await getExpenseCategoryById(provider.id, franchise_id, id);
         if (!expense_category) {
         ...
         try{
          const deleted_expense_category = await deleteExpenseCategoryById(provider.id , id);
         return returnResponse(res, StatusCodes.OK, `expense_category deleted successfully`, deleted_expense_category);
         }
          catch(err){
          logger.error(`--- Cannot delete expense category as it is linked to other records ---`);
          return returnError(res, StatusCodes.BAD_REQUEST, `Cannot delete expense category as it is linked to other records`);
         }
...
export  {deleteExpenseCategoryByIdEndpoint}